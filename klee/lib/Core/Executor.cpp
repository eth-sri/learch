...
cl::opt<bool> NoEarlyStates(
    "no-early-states",
    cl::init(false),
    cl::desc("Do not output early terminated states (default=false)"),
    cl::cat(TestGenCat));
...
cl::list<Executor::TerminateReason> ExitOnErrorType(
    ...
    cl::values(
        ...
        clEnumValN(Executor::ShiftOutOfBounds, "ShiftOutOfBounds", "Shift out of bounds"),
        clEnumValN(Executor::OutOfBounds, "OutOfBounds", "Array index out of bounds"),
        clEnumValN(Executor::PointerOverflow, "PointerOverflow", "Pointer overflow"),
        clEnumValN(Executor::NullDereference, "NullDereference", "Null dereference"),
...
cl::opt<bool> KillBySearcher(
    "kill-by-searcher",
    cl::desc(
        "Use ML searcher to kill states when over memory cap (default=false)"),
    cl::init(false),
    cl::cat(TerminationCat));

cl::opt<bool> TerminateAnyBug(
    "terminate-any-bug",
    cl::desc(
        "Terminate KLEE when any bug is found"),
    cl::init(false),
    cl::cat(TerminationCat));

cl::opt<unsigned> RNGSeed(
    "rng-initial-seed",
    cl::desc("the random seed of RNG"),
    cl::init(5489UL),
    cl::cat(SeedingCat));
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
                   InterpreterHandler *ih, bool _featureExtract)
    : Interpreter(opts), featureExtract(_featureExtract), interpreterHandler(ih), searcher(0),
  ...
  std::vector<subpathCount_ty> vec(4);
  subpathCounts = vec;
...
Executor::setModule(std::vector<std::unique_ptr<llvm::Module>> &modules,
  ...
  // int num_inst = 0;
  // for (llvm::Module::iterator f = kmodule->module.get()->begin(), e = kmodule->module.get()->end(); f != e; ++f) {
  ...
  //   }
  // }
  // std::cout << "num_inst: " << num_inst << " " << std::endl << std::flush;

  // InterProcCFG* cfg = new InterProcCFG(kmodule->module.get());
  // cfg->computeScore();
  // for (InterProcCFG::iterator n = cfg->begin(); n != cfg->end(); n++) {
  //   BlockStat* stat = n->second->getStat();
  //   if (stat && n->second->getBasicBlock()) {
  //     stat->dump(n->second->getBasicBlock());
  //   }
  // }

  // cfg->dump("graph");
  // for (llvm::Module::iterator f = kmodule->module.get()->begin(); f != kmodule->module.get()->end(); f++) {
  //   for (llvm::Function::iterator b = f->begin(); b != f->end(); b++) {
  //     llvm::Instruction* inst = &*(b->begin());
  //     llvm::MDNode* n = inst->getMetadata("num_source");
  //     if (n) {
  //       unsigned num_source = cast<llvm::ConstantInt>(dyn_cast<llvm::ConstantAsMetadata>(n->getOperand(0))->getValue())->getSExtValue();
  //       std::cout << "num_source: " << &*b << " " << inst << " " << num_source << std::endl;
  //     }
  //   }
  // }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Call: {
    ...
    // if (f) {
    //   std::cout << i->getParent()->getParent()->getName().str() << " calls " << f->getName().str() << " " << state.stack.size() << std::endl;
    // } else {
    //   std::cout << "symbolic call " << state.stack.size() << std::endl;
    // }
...
void Executor::updateStates(ExecutionState *current) {
  ...
  // if (featureExtract) {
  //   for (auto it=featureStates.begin(); it!=featureStates.end(); ++it) {
  //     (*it)->predicted = false;
  //     getStateFeatures(*it);
  //   }
  //   featureStates.clear();
  ...
  if ((stats::instructions & 0xFFFF) == 0) {
    ...
    if (mbs > MaxMemory) {
      if (mbs > MaxMemory + 100) {
        ...
        klee_warning("killing %d states out of %d (over memory cap)", toKill, numStates);
        std::vector<ExecutionState *> arr(states.begin(), states.end());
        if (KillBySearcher) {
          std::sort(arr.begin(), arr.end(), ExecutionScoreCompare());
          for (unsigned i = 0; i < toKill; ++i) {
            terminateStateEarly(*arr[i], "Memory limit exceeded.");
          }
        } else {
...
std::vector<std::pair<std::string, double>> Executor::getFeatures() {
    std::vector<std::pair<std::string, double>> toolFeatures;
    unsigned totalFuncs = kmodule->functions.size();
    unsigned totalArgs = 0;
    unsigned totalRegs = 0;
    unsigned totalInsts = 0;
    unsigned totalBBs = 0;
    for(auto & function : kmodule->functions) {
        KFunction *testi = function.get();

        totalArgs += testi->numArgs;
        totalRegs += testi->numRegisters;
        totalInsts += testi->numInstructions;
        totalBBs += testi->basicBlockEntry.size();
    }

    toolFeatures.emplace_back("num_funcs", totalFuncs);
    toolFeatures.emplace_back("num_bbs", totalBBs);
    toolFeatures.emplace_back("num_insts", totalInsts);
    toolFeatures.emplace_back("args_per_func", totalArgs * 1. / totalFuncs);
    toolFeatures.emplace_back("regs_per_func", totalRegs * 1. / totalFuncs);
    toolFeatures.emplace_back("insts_per_func", totalInsts * 1. / totalFuncs);
    toolFeatures.emplace_back("insts_per_bb", totalInsts * 1. / totalBBs);
    toolFeatures.emplace_back("bbs_per_func", totalBBs * 1. / totalFuncs);
    return toolFeatures;
}

void Executor::processFeatures() {
    interpreterHandler->processFeatures(getFeatures());
}
...
void Executor::getSubpath(ExecutionState* state, subpath_ty &result, uint index) {
  ...
  uint length = 1 << index;
  length = length < state->takenBranches.size()? length : state->takenBranches.size();
  for(auto it = state->takenBranches.rbegin(); it != state->takenBranches.rbegin() + length; it++) {
...
unsigned getBlockMetadata(BasicBlock* block, std::string feature_type) {
  Instruction* inst = &*(block->begin());
  MDNode* n = inst->getMetadata(feature_type);
  if (n) {
    unsigned feature = cast<ConstantInt>(dyn_cast<ConstantAsMetadata>(n->getOperand(0))->getValue())->getSExtValue();
    return feature;
  } else {
    return 0;
  }
...
void Executor::getStateFeatures(ExecutionState *es) {
  ...
  double query_cost = es->queryCost.toSeconds();
  for (uint i=0; i<es->features.size(); i++) {
  ...
  double blockVisitTime = 0;
  if (ExecutionState::blockVisitTimes.find(block) != ExecutionState::blockVisitTimes.end()) {
    blockVisitTime = ExecutionState::blockVisitTimes[block];
  }

  // double num_block = getBlockMetadata(block, "num_block");
  // double num_inst = getBlockMetadata(block, "num_inst");
  // double num_source = getBlockMetadata(block, "num_source");
  // double num_bug = getBlockMetadata(block, "num_bug");
  // double num_overflow = getBlockMetadata(block, "num_overflow");
  // double num_shift = getBlockMetadata(block, "num_shift");
  // double num_oob = getBlockMetadata(block, "num_oob");
  // double num_pointer = getBlockMetadata(block, "num_pointer");
  // double num_null = getBlockMetadata(block, "num_null");
  ...
  es->feature.push_back(query_cost);
  es->feature.push_back(es->queryCost.toSeconds());
  es->feature.push_back(es->coveredInsts.size());
  es->feature.push_back(es->coveredSource.size());
  // es->feature.push_back(es->coveredBlocks.size());
  es->feature.push_back(newCoveredInsts);
  es->feature.push_back(newCoveredSource);
  // es->feature.push_back(newCoveredBlocks);
  // es->feature.push_back(blockVisitTime);
  // es->feature.push_back(num_block);
  // es->feature.push_back(num_inst);
  // es->feature.push_back(num_source);
  // es->feature.push_back(num_bug);
  // es->feature.push_back(num_overflow);
  // es->feature.push_back(num_shift);
  // es->feature.push_back(num_oob);
  // es->feature.push_back(num_pointer);
  // es->feature.push_back(num_null);
  es->feature.push_back(depth);
  es->feature.push_back(es->stack.size());
  es->feature.push_back(ExecutionState::genTestCases);
  es->feature.push_back(inst_count);
  es->feature.push_back(cp_inst_count);
  es->feature.push_back(num_succ);
  es->feature.push_back(instsSinceCovNew);
  es->feature.push_back(sgs1);
  es->feature.push_back(sgs2);
  es->feature.push_back(sgs4);
  es->feature.push_back(sgs8);
  for (auto it = constraint_features.begin(); it != constraint_features.end(); it++) {
    es->feature.push_back(*it);
  }
}

void Executor::doDumpStates() {
  doDumpStates(states, true);
}

void Executor::doDumpStates(std::set<ExecutionState *, ExecutionStateIDCompare> &statesToDump, bool doUpdateStates) {
  if (!DumpStatesOnHalt || states.empty() || statesToDump.empty())
    return;

  klee_message("halting execution, dumping remaining states");
  for (const auto &state : statesToDump)
...
//                          External);
//    return;
    klee_warning_once(function, "external modified read-only object (originally terminated state)");
  }
...
Interpreter *Interpreter::create(LLVMContext &ctx, const InterpreterOptions &opts,
                                 InterpreterHandler *ih, bool featureExtract) {
  return new Executor(ctx, opts, ih, featureExtract);
}
