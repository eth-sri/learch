...
public:
  ...
  std::vector<std::pair<std::string, double>> getFeatures();
...
private:
  ...
  void doDumpStates();
  void doDumpStates(std::set<ExecutionState *, ExecutionStateIDCompare> &states, bool doUpdateStates);
...
public:
  Executor(llvm::LLVMContext &ctx, const InterpreterOptions &opts,
      InterpreterHandler *ie, bool featureExtract);
  virtual ~Executor();
...
struct ExecutionScoreCompare {
  bool operator()(const ExecutionState *a, const ExecutionState *b) const {
    if (a->predicted && !b->predicted) {
      return true;
    } else if (!a->predicted && b->predicted) {
      return false;
    } else if (a->predicted && b->predicted) {
      return a->predicted_reward < b->predicted_reward;
    } else {
      return true;
    }
  }
