...
#define addDNR(name, handler) { name, \
  ...
  // Run clang with -fsanitize=shift
  ...
  // Run clang with -fsanitize=null
  add("__ubsan_handle_type_mismatch", handleNullDereference, false),
  add("__ubsan_handle_type_mismatch_abort", handleNullDereference, false),
  add("__ubsan_handle_type_mismatch_v1", handleNullDereference, false),
  add("__ubsan_handle_type_mismatch_v1_abort", handleNullDereference, false),
...
void SpecialFunctionHandler::handleShiftOutOfBounds(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
  executor.terminateStateOnError(state, "shift out of bounds",
                                 Executor::ShiftOutOfBounds);
}

void SpecialFunctionHandler::handleOutOfBounds(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
  executor.terminateStateOnError(state, "array index out of bounds",
                                 Executor::OutOfBounds);
}

void SpecialFunctionHandler::handlePointerOverflow(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
  executor.terminateStateOnError(state, "pointer overflow",
                                 Executor::PointerOverflow);
}

void SpecialFunctionHandler::handleNullDereference(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
  auto pointerExpr = arguments[1].get();
  auto pointer =
       executor.toConstant(state, pointerExpr, "handleTypeMismatch");
  assert(pointer->isZero() && "non null dereference type mismatch");
  executor.terminateStateOnError(state, "null dereference",
                                 Executor::NullDereference);
}
