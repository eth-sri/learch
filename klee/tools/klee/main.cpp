...
namespace {
  ...
  cl::opt<bool >
  OutputDirN("output-dir-n",
          cl::desc("append a unique integer n to the output director<"),
          cl::init(false),
          cl::cat(StartCat));
  ...
  cl::opt<bool>
  DumpElocs("dump-elocs",
      cl::desc("Dump the list of ELOCs"),
      cl::init(false),
      cl::cat(DebugCat));
}
...
public:
  ...
  void processFeatures(const std::vector<std::pair<std::string, double>> &features);
  ...
  if (dir_given) {
    if(OutputDirN) {
      ...
      for (; i <= INT_MAX; ++i) {
        ...
        //llvm::sys::path::append(d, directory.c_str());
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!WriteNone) {
    ...
    if (success) {
      if(state.coveredNew) ++ExecutionState::genTestCases;
      ...
      if (!kTest_toFile(&b, getOutputFilename(getTestFilename("ktest", id)).c_str())) {
        ...
        ++m_numGeneratedTests;
        auto testTime = std::time(nullptr);
        std::stringstream testMsg;
        testMsg << getTestFilename("ktest", id) << ": " << std::put_time(std::localtime(&testTime), "%Y-%m-%d %H:%M:%S");
        klee_message("%s", testMsg.str().c_str());
      }
    ...
    if (FeatureDump && !state.features.empty()) {
        ...
        if(f) {
            ...
            for(const std::pair<long, std::vector<double>> &pair : state.features) {
...
void KleeHandler::processFeatures(const std::vector<std::pair<std::string, double>> &features) {
        auto f = openOutputFile("constant_features.csv");
        if(f) {
            const auto& pEnd = *features.rbegin();
            ...
            *f << "\n";
            for(const auto& p: features) {
                std::stringstream tmp;
                tmp << std::setprecision(3) << std::fixed << p.second;
                *f << tmp.str();
                if(p != pEnd) {
                    *f << ",";
                }
...
int main(int argc, char **argv, char **envp) {
  ...
  { // output clock info and start time
    ...
    startMsg << "Started: " << std::put_time(std::localtime(&startTime), "%Y-%m-%d %H:%M:%S");
    klee_message("%s", startMsg.str().c_str());
  }

  if (DumpElocs) {
    for (llvm::Module::iterator f = finalModule->begin(), e = finalModule->end(); f != e; ++f) {
      for (llvm::Function::iterator b = f->begin(); b != f->end(); ++b) {
        for (llvm::BasicBlock::iterator iit = b->begin(); iit != b->end(); ++iit) {
          const llvm::DebugLoc& debugLoc = iit->getDebugLoc();
          if (debugLoc.get() != nullptr) {
            std::string directory = debugLoc->getDirectory();
            std::string path = debugLoc->getFilename();
            int line = debugLoc->getLine();
            std::stringstream msg;
            msg << "Eloc: " << directory << " " << path << " " << line;
